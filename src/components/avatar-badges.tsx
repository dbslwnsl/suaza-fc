 ...
 * - 직책(titleBadges): 좌상단, 아래로 누적
 * - 수상(awardBadges): 우하단, 위로 누적
 *
...
export default function AvatarBadges({
  ...
  awardBadges = [],
  size = "sm",
  awardClassName = "-bottom-1 -right-1",
}: {
  titleBadges?: MemberBadge[];
  awardBadges?: MemberBadge[];
  size?: "xs" | "sm";
  /** 수상 뱃지 컨테이너 위치 오버라이드 (카메라 어포던스와 겹치지 않게) */
  ...
  return (
    <>
      ...
      {awardBadges.length > 0 && (
        <div
          className={`absolute z-10 flex flex-col items-end gap-0.5 pointer-events-none ${awardClassName}`}
        >
